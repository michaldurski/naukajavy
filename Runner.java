...
public class Runner {
		...
		while (true) {
			try {
				...
				if (tmp.equals("graphics"))
					pdfGraphics();
				if (tmp.equals("savetopdf"))
					saveTo1PDF(lista);
				if (tmp.equals("savetomanypdf"))
					saveToManyPDFs(lista);
				if (tmp.equals("savetomanytxt"))
	...
	private static void saveToManyTXTs(ArrayList<Dokument> lista) throws IOException {
		for( int i = 0 ; i<lista.size();i++){
			File plik = new File(i+1+"_"+lista.get(i).getTytul());
			FileWriter plikWriter = new FileWriter(plik);
			BufferedWriter buffPliku= new BufferedWriter(plikWriter);
			
			buffPliku.write(lista.get(i).saveMitopdf());
			buffPliku.close();
			
			
		}
		System.out.println("pliki zapisane do wielu plikow tekstowych");
	}
	...
	private static void saveTo1PDF( ArrayList<Dokument> lista) throws IOException {
		...
		try {
			...
			document.open();
			for (int i = 0; i < lista.size(); i++) {
				document.add(new Paragraph("dokument nr " + i));
				document.add(new Paragraph(lista.get(i).getTytul() + " => " + lista.get(i).getTresc()));
			}
			document.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	private static void pdfGraphics() {
		try {
			...
			/*
			 * Map map = mapper.getMapper(); for (Iterator i =
			 * map.keySet().iterator(); i.hasNext();) { String name = (String)
